...
def closing_moment_card(item: Dict[str, Any]) -> str:
    ...
    return f"""
    <div class="moment" style="grid-template-columns: 34px minmax(0, 1fr);">
      <div class="moment-icon">✕</div>
      <div>
        <div class="moment-quote">{esc(item.get('reason', ''))}</div>
        {dialogue}
        <div class="moment-miss">Score: {esc(item.get('score', '—'))}/100 · Strategy used: {esc(strategy)}</div>
        <div class="user-chip">User ID: {esc(user_id)}</div>
      </div>
    </div>
    """
...
def render_home(agent: Dict[str, Any]) -> None:
    ...
    section_title(
        ...
        f"{agent.get('worst_closing_total', 0)} legitimate examples",
    )
    ...
    focus_card(1, "Low-scoring closes on real, full-length calls", "Each of these had a genuine chance to close — the call ran its full length, not cut short by the owner.", len(worst), rows_html)
