...
st.set_page_config(page_title="Owner Sales Performance", page_icon="📞", layout="centered")
...
def inject_css():
    st.markdown(
        ...
        div.block-container {{ max-width: 480px; padding-top: 1.5rem; }}
        .greet {{ font-size: 13px; color: {SLATE}; }}
        .greet b {{ color: {NAVY}; font-size: 20px; display:block; margin-top:2px; }}
        .pill {{ background: #F3EEFE; color: {VIOLET_DK}; font-size: 11px; font-weight: 700;
                 padding: 4px 11px; border-radius: 20px; display:inline-block; margin-top: 8px; }}
        ...
        .strength-item {{ display:flex; gap:10px; padding:8px 0; border-top:1px solid #EAEAF2; font-size:13.5px; line-height:1.4; }}
        .strength-item:first-of-type {{ border-top:none; }}
        .moment {{ background:#F7F7FB; border-radius:10px; padding:12px 14px; margin-bottom:8px; font-size:12.5px; }}
        .moment .owner {{ font-style:italic; color:{NAVY}; }}
        .moment .miss {{ color:{RED}; margin-top:4px; }}
        .cdr-tag {{ font-family:monospace; font-size:10px; background:#EEF0F6; color:{SLATE};
                    padding:2px 7px; border-radius:20px; }}
        .sec-label {{ font-size:11px; text-transform:uppercase; letter-spacing:.06em; color:{SLATE};
                      font-weight:700; margin:20px 2px 8px; }}
        .footnote {{ font-size:11px; color:{SLATE}; line-height:1.5; margin-top:10px; padding-top:10px;
...
# ── COMPONENTS ───────────────────────────────────────────────────────────
def score_ring(score: int, label="Overall performance"):
    ...
    fig.update_layout(
        ...
        annotations=[dict(text=f"<b>{score}</b><br><span style='font-size:11px;color=#6B7280'>out of 100</span>",
                           x=0.5, y=0.5, showarrow=False, font=dict(size=30, color=NAVY))],
    )
...
def render_home(agent):
    score_ring(agent["overall_score"])
    st.caption("Based only on what they said or did on calls")
    ...
    with st.expander(f"2️⃣ Offer the plan when the owner asks directly — {agent['area_b_total']} this cycle", expanded=False):
        if agent["area_b"]:
...
def render_trends(agent):
    ...
    if agent["good_example"]:
        ...
        st.markdown('<div class="sec-label">A rebuttal that worked — reuse this</div>', unsafe_allow_html=True)
        st.markdown(
            f"""<div class="card">
            <b>{(g['sop'] or '').split(':')[0]}</b><br>
            <span style="color:{SLATE};">Owner:</span> {g['objection'] or ''}<br>
...
def render_profile(agent):
    ...
    st.markdown(
        ...
        <h3>Manager note</h3>
        <p style="font-size:13px;color:{SLATE};">Two areas flagged this cycle by automated analysis of
...
def render_login(agents, directory):
    st.markdown(
        ...
        <h1 style="font-size:22px;margin:0 0 6px;">Owner Sales Performance</h1>
        <p style="color:{SLATE};font-size:13.5px;">Enter your name or employee ID to see your own
