...
MANAGER_PASSCODE = "saathi-admin"  # change this to whatever you want the manager login phrase to be
...
st.set_page_config(page_title=f"{APP_NAME} — Owner Sales", page_icon="🤝", layout="wide")
...
def inject_css():
    st.markdown(
        ...
        div[data-testid="stVerticalBlockBorderWrapper"] {{
            background: #fff; border-radius: 16px !important; border: none !important;
            box-shadow: 0 1px 4px rgba(20,20,50,0.06);
        }}
        div[data-testid="stExpander"] {{
            background:#fff; border-radius:16px !important; border: none !important;
            box-shadow: 0 1px 4px rgba(20,20,50,0.06); margin-bottom: 16px;
        }}

        /* hide the auto anchor-link icon Streamlit adds to any heading tag */
        [data-testid="stHeaderActionElements"] {{ display: none !important; }}

        .card-title {{ font-size: 16px; font-weight: 700; color: {NAVY}; margin-bottom: 4px; }}
        ...
        .strength-item {{ font-size:14px; line-height:1.5; padding: 4px 0; }}
        .moment {{ background:#F7F7FB; border-radius:10px; padding:14px 16px; margin-bottom:6px; font-size:13.5px; }}
        ...
        .footnote {{ font-size:12px; color:{SLATE}; line-height:1.6; }}
        ...
        /* small monospace CDR id blocks (st.code) */
        .stCodeBlock {{ margin-bottom: 10px !important; }}
        .stCodeBlock pre {{ padding: 4px 10px !important; min-height: 0 !important; font-size: 11px !important; }}
        </style>
        """,
...
# ── COMPONENTS ───────────────────────────────────────────────────────────
def score_ring_figure(score: int):
    color = GREEN if score >= 65 else (AMBER if score >= 35 else RED)
    fig = go.Figure(
        go.Pie(
            values=[score, 100 - score], hole=0.78,
            marker_colors=[color, "#EFEFF4"], textinfo="none",
            sort=False, direction="clockwise", rotation=90,
        )
    )
    fig.update_layout(
        showlegend=False, margin=dict(l=0, r=0, t=0, b=0), height=170, width=170,
...
def render_score_card(score, conv_q, conv_s):
    with st.container(border=True):
        ...
        with ring_col:
            st.plotly_chart(score_ring_figure(score), use_container_width=False, config={"displayModeBar": False})
        with metrics_col:
...
def render_strengths_card(strengths):
    with st.container(border=True):
        st.markdown('<div class="card-title">👍 What\'s genuinely working</div>', unsafe_allow_html=True)
        for s in strengths:
...
def trend_chart(points):
    if len(points) < 2:
        st.caption(
            "Not enough day-to-day spread in this cycle's data to chart a reliable daily trend yet — "
            "will build out once more cycles run."
        )
        return
...
def moment_card_a(item):
    ...
    st.code(item["cdr_id"], language=None)
        ...
        if resp.status_code == 200:
            try:
                ...
                if body.get("status") == "ok":
                    return True
                st.error(f"Sheet responded but didn't confirm success: {body}")
                return False
            except ValueError:
                st.error(
                    "Got an unexpected response from the feedback sheet (not JSON) — this usually means "
                    "the Apps Script deployment's 'Who has access' isn't set to Anyone, or it needs "
                    "redeploying. Feedback was NOT saved."
                )
                return False
        else:
...
def feedback_widget(key, agent_id, area_title):
    ...
    if c1.button("👍 Like", key=f"{key}_like", use_container_width=True):
        st.session_state[reaction_key] = "like"
    if c2.button("👎 Dislike", key=f"{key}_dislike", use_container_width=True):
...
def render_home(agent):
    ...
    with left:
        render_score_card(agent["overall_score"], agent["conversation_quality"], agent["conversion_score"])
    with right:
        render_strengths_card(agent["strengths"])
...
def render_trends(agent):
    ...
    with col1:
        ...
        with st.container(border=True):
            ...
            st.caption(
                "Score = 50% rebuttal compliance + 50% pitch-completion rate that day, for this agent "
                "only. Days with fewer than 5 relevant calls are skipped. This chart covers one review "
                "cycle — each future cycle adds another point, building toward a real trend."
            )
    ...
    with col2:
        if agent["good_example"]:
            ...
            with st.container(border=True):
                st.markdown(f"**{(g['sop'] or '').split(':')[0]}**")
                st.markdown(f"<span style='color:{SLATE};'>Owner:</span> {g['objection'] or ''}", unsafe_allow_html=True)
                st.markdown(f"<span style='color:{GREEN};font-weight:600;'>Reply:</span> {g['rebuttal'] or ''}", unsafe_allow_html=True)
                st.code(g["cdr_id"], language=None)

    st.markdown('<div class="sec-label">A note on this cycle\'s data</div>', unsafe_allow_html=True)
    with st.container(border=True):
        st.markdown(
            "**This cycle's areas were auto-detected**, not manually transcript-reviewed. "
            "Use the 👍/👎 feedback on Home to flag anything that looks wrong.\n\n"
            "**Customer intent / language match not scored** — that's the AI analyser's read of the call, "
            "not something the agent controls."
        )
...
def render_profile(agent):
    ...
    with header_col:
        ...
        st.markdown(
            ...
            <div style="font-size:19px; font-weight:700; color:{NAVY};">{agent['name']}</div>
    ...
    with st.container(border=True):
        st.markdown('<div class="card-title">Manager note</div>', unsafe_allow_html=True)
        st.markdown(
            f"Two areas flagged this cycle by automated analysis of {agent['n_live_calls']} live calls "
            "— review the 👍/👎 feedback before the next 1:1."
        )
        st.caption(
            "The call analyser tags things like customer intent by reading the call — that's not "
            "something the agent controls, so it's excluded from the score. Everything else is their "
            "own words and actions, each traceable to a call ID."
        )
...
def render_login(agents, directory):
    ...
    with mid:
        st.markdown(
            ...
            <div style="font-size:26px; font-weight:700; margin:0 0 8px; color:{NAVY};">{APP_NAME}</div>
            <p style="color:{SLATE};font-size:14px;">Your calls, your progress — just for you.<br>
            Type your name or employee ID to see how this cycle went.</p>
            </div>""",
            unsafe_allow_html=True,
        )
...
def render_manager_view():
    ...
    with mid:
        st.markdown(
            f"""<div style="text-align:center; padding: 60px 0 20px;">
            <div style="font-size:24px; font-weight:700; color:{NAVY};">Manager view</div>
            <p style="color:{SLATE};font-size:14px;">Feedback from every agent's 👍/👎 lands in your sheet in real time.</p>
            </div>""",
            unsafe_allow_html=True,
        )
...
def main():
    ...
    with top_l:
        st.markdown(
            f'<div class="greet">Hey<b>{agent["name"]}</b>'
            f'<span class="pill">🤝 Here\'s how this cycle went</span></div>',
            unsafe_allow_html=True,
        )
    with top_r:
